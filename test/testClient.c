...
int main(int argc, char *argv[])
{
    ...
    for (int i = 5; i < 10; i++)
        fd[i] = udp_client_init(UDP_IP, UDP_PORT);
    ...
    while (42) {
        ...
        for (int i = 5; i < 10; i++)
            udp_client_send(fd[i], send_buf, strlen(send_buf));
        ...
        for (int i = 5; i < 10; i++) {
            ret = udp_client_recv(fd[i], send_buf, sizeof(send_buf), addr, &port);
            printf("\n fd: %d, receive %d bytes from %s:%d: %s", fd[i], ret, addr, port, recv_buf);
        }
